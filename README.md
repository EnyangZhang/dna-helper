...
普通扼守、普通驱离和密函驱离使用左下角角色血条作为状态边界。无论技能是否开启，局外监控都会检查血条；连续 3 帧命中后切入局内，局内血条消失后切回局外。血条边界检测与“是否释放技能”是两件事：技能关闭时仍用血条分区，但不会进入技能链。

同一状态中的多个候选按优先级快速轮询，这里的“并行检测”不是多个线程。每个状态只包含自己的按钮，不会在局内误点“再次进行 / 开始挑战”，也不会在局外误点“继续挑战 / 确认选择”。
...
驱离和扼守允许从任意页面启动，因此任务刚启动、状态尚未知时会先判断血条和当前可能出现的结算按钮；完成首次归类后严格按上述列表分区。技能结束后的监控保留“本副本已释放”状态，即使血条短暂消失又恢复也不会重复释放。
...
局内不识别任何确认按钮；血条消失后切换到局外，才开始识别“再次进行”。识别后按与扼守相同的完成轮次语义计数；未达到总轮数时，依次点击“再次进行”和“开始挑战”进入下一轮。
