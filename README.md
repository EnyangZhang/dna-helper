...
7. “后台发送技能键（实验）”只提供给密函驱离和普通驱离，默认关闭；普通扼守固定使用前台技能输入。驱离开启实验项后仅 E/Q 通过 Windows 后台窗口消息投递，不切换到游戏前台，也不移动鼠标；所有按钮点击仍使用前台控制。日志中的“已投递”只表示 Windows 接受了消息，游戏若使用 Raw Input、DirectInput 或自身输入层仍可能忽略，因此未实际释放时应关闭此项。
...
控制器默认执行真实前台输入。Python Agent 会持续记录用户正在操作的非游戏窗口及鼠标屏幕坐标；完成一组前台输入后先释放鼠标限制，再尝试切回该窗口，并在窗口恢复成功时把鼠标移回记录位置。坐标使用 Windows 虚拟屏幕坐标，因此支持位于主屏左侧或上方的显示器负坐标。密函驱离和普通驱离的实验性后台技能键不触发这套切换与恢复流程；普通扼守始终执行前台技能输入。
