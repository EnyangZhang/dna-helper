...
同一状态中的多个候选按优先级快速轮询，这里的“并行检测”不是多个线程。正常局内业务监控只包含自己的局内按钮；进入边界未知等待后先检查血条，血条仍不存在时才把“再次进行 / 开始挑战 / 副本外 Space 确认”作为局外证据。局外监控不识别“继续挑战 / 确认选择”。
...
驱离和扼守允许从任意页面启动，因此任务刚启动、状态尚未知时会先判断血条和当前可能出现的结算按钮；完成首次归类后严格按上述列表分区。局内血条短暂消失时进入无超时的边界未知等待：血条恢复就返回原局内监控，局外专属按钮出现才进入局外流程。技能结束后的监控始终保留“本副本已释放”状态，血条短暂消失又恢复不会重复释放。
...
任务启动、HUD 确认、技能释放和点击“开始挑战”均不会增加轮次。技能关闭时只处理局内“继续挑战”“确认选择”和局外重开；技能开启时，每轮战斗开始后额外执行一次 E/Q 组合。技能结束后仍在独立的局内监控中处理“继续挑战”和“确认选择”，但同一副本不会再次进入技能链。只有完成“再次进行”和“开始挑战”进入下一轮后才重新允许释放。无论技能是否开启，从“再次进行”页面启动任务都会先识别该页面并按完成轮次语义处理，不会卡在 HUD 等待。
