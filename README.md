...
任务启动、HUD 确认、技能释放和点击“开始挑战”均不会增加轮次。技能关闭时只处理局内“继续挑战”“确认选择”和结算重开；技能开启时，每轮战斗开始后额外执行一次 E/Q 组合。无论技能是否开启，从“再次进行”页面启动任务都会先识别该页面并按完成轮次语义处理，不会卡在 HUD 等待。
...
技能关闭时只处理结算与重开；技能开启时，每个副本只执行一次 E/Q 组合，之后只监听“再次进行”，不会在同一副本重复释放。两种状态都支持直接从“再次进行”页面启动。
