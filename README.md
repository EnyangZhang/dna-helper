...
```text
OK: 1 controller(s), 2 group(s), 3 task(s), 2 preset(s), 122 reachable pipeline node(s), 9 template(s)
```
...
“继续挑战”三连击后先进入 1500ms 的专用冷却节点，再恢复对应的局内监控。即使页面动画期间按钮残影仍被再次识别，Agent 也会忽略 5 秒内同一任务重复上报的局内轮次事件，因此不会重复增加 Telegram 进度；重试点击本身仍保留，以便第一次输入未生效时继续推进页面。
...
“继续挑战”点击后同样经过 1500ms 冷却，并对 5 秒内的重复进度事件去重，避免按钮残影造成局内轮次虚增。
...
7. “后台发送技能键（实验）”只提供给密函驱离和普通驱离，默认关闭；普通扼守固定使用前台技能输入。驱离开启实验项后，E/Q 通过 Windows 后台窗口消息投递，所有按钮点击仍使用前台控制。每个新任务首次后台发送前，如果本任务尚未观察到游戏获得前台，Agent 会短暂激活游戏，再恢复用户原窗口和鼠标位置，用于初始化 Unreal 的输入状态；之后的 E/Q 不再切换前台。若 Windows 明确拒绝消息，会自动重新初始化并重试一次。日志中的“已投递”仍只表示 Windows 接受了消息，游戏若使用 Raw Input、DirectInput 或自身输入层仍可能忽略，因此持续未实际释放时应关闭此项。
...
控制器默认执行真实前台输入。Python Agent 会持续记录用户正在操作的非游戏窗口及鼠标屏幕坐标；完成一组前台输入后先释放鼠标限制，再尝试切回该窗口，并在窗口恢复成功时把鼠标移回记录位置。坐标使用 Windows 虚拟屏幕坐标，因此支持位于主屏左侧或上方的显示器负坐标。密函驱离和普通驱离的实验性后台技能键平时不触发切换与恢复；但每个新任务首次投递前若游戏尚未获得过前台，会自动执行一次短暂激活与恢复。普通扼守始终执行前台技能输入。
