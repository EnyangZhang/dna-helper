...
任务启动、HUD 确认、技能释放和点击“开始挑战”均不会增加轮次。技能关闭时只处理局内“继续挑战”“确认选择”和结算重开；技能开启时，每轮战斗开始后额外执行一次 E/Q 组合。技能结束后切换到不包含 HUD 的局内/结算监控，仍会点击“继续挑战”和“确认选择”，但同一轮不会再次进入技能链。只有完成“再次进行”和“开始挑战”进入下一轮后才重新允许释放。无论技能是否开启，从“再次进行”页面启动任务都会先识别该页面并按完成轮次语义处理，不会卡在 HUD 等待。
...
技能关闭时只处理结算与重开；技能开启时，每个副本只执行一次 E/Q 组合，之后切换到不包含 HUD 的纯结算监控，只监听“再次进行”，不会在同一副本重复释放。点击“开始挑战”进入下一轮后才重新允许释放。两种状态都支持直接从“再次进行”页面启动。
