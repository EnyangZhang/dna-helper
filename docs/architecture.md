...
→ 开关开启：CipherEndlessRewardPage 以原第一页确认按钮为入口，不识别标题或编号
→ DoNothing + post_delay 1000ms：等待奖励加载，不点击；随后以新帧重新识别
→ 从左到右取第一个红色目标；没有红色或第 1 张命中：直接确认默认项
→ 最左红色在第 2/3 张：对应槽位原生三连击 → 无输入恢复 → 直接进入确认按钮识别
→ “确认选择”仍存在：原生三连击；不存在则返回等待，不检查勾选
→ “继续挑战”三连击
→ 第三页“Space 确认选择”三连击
→ RewardConfirmEntry
```
...
奖励选卡独立定义在 `CipherRewardChoice.json`，识别器 `agent/cipher_rewards.py` 只返回图像证据，不发输入、不改计数、不保存上一次的选卡状态。`page` 模式只以原第一页确认按钮（原色、原 ROI、阈值 0.80）作为入口。运行时只加载确认按钮、红色图标两个模板，不再匹配标题或 I/II/III 编号，也没有 `card_layout` 门槛，避免金色等稀有度外观阻断识别。三个固定槽位按左到右产生 `red_slots`，`target_slot` 为其第一项，没有目标则为 None。`ready` 只在 target=None 或 1 时直接确认默认项；target=2/3 必须先走对应的 `select` 原生三连击，完全不读取勾选。选卡收尾的 next 优先检查“再次进行”，随后转入独立的 `CipherEndlessRewardConfirmAfterSelect`；该节点只匹配原确认按钮（同 ROI、阈值），首击后复用 `CipherEndlessRewardConfirmClick2/3` 与无输入收尾，不再调用选卡识别器。多个红色同样取最左目标，原来的勾选状态不改变优先级。第 1 张选卡链已删除，2/3 坐标分别为 `(640,519)`、`(823,519)`。缺失确认按钮、两个运行时模板之一或非法尺寸仍不点击。红色图标原色归一化相关阈值 0.85，客户区严格为 1280×720。识别详情附带目标槽位、确认按钮分数、红色槽位及三项分数；拒绝画面还附带确认按钮未命中、模板缺失或格式错误的原因。

`RewardRecognitionLog` 将上述证据输出到运行日志，前缀为 `[密函选卡]`。只输出 `ready` 的实际新帧判断及 `page/ready` 的拒绝原因；成功的等待前 `page` 和重复 `select` 候选保持安静，不能将待加载画面的默认回退显示成最终判断。日志明确显示最左目标和直接确认/点击第 N 张后直接确认，不再输出标题、编号或勾选分数。按 task_id、命中槽位、目标和决策做语义去重：变化即输出，分数抖动不触发；同一奖励页判断每 10 秒刷新一次，相同的非奖励页拒绝只输出一次。单个有锁内存记录限制状态大小，任务变化重置；去重记录不参与识别、选卡、轮次或状态机路由。日志不能宣称点击成功；日志管道关闭不影响识别结果。只有开启选卡的密函无尽路径可达，关闭开关、密函驱离及其他任务无新增诊断输出。回归固定分数证据、拒绝原因、去重刷新、加载前不误报和日志失败不改变识别结果。

第 2/3 张选卡与新的无尽确认入口都有独立的 `Custom recognition + Click → DirectHit + Click → DirectHit + Click → focus_guard_finalize` 链，框架时序分别为 `0/0/50、0/0/50、0/0/0、0/0/0`，无输入收尾保留原有 100ms 焦点恢复等待。首个节点外不重复识别；选卡收尾之后只检测确认按钮并直接确认，不校验游戏是否显示勾选，也不新增缓存选中状态。确认按钮未出现或奖励页残留经 `CipherEndlessRewardWait` 的 50ms 空闲节点重试；仅开关开启时 `RewardConfirmWaitContinue` 才检查奖励页以恢复未成功确认的页面。各等待/收尾先检测 `CipherEndlessAgainDetected`，并允许正常推进到“继续挑战”。三连击内部保持短链，不插入识别或 Agent 页面输入。没有新轮次事件，原 `cipher_cycle_completed` 仍只在第三页收尾发送。

`CipherEndlessRewardPage` 的时序是有文档约束的例外 `0/0/1000`：确认按钮会早于奖励图标出现，因此该无输入门控先等待 1000ms，再由后续 Custom 识别节点在新帧重新决定选卡或默认确认。不能把等待加在确认 Click 的 pre_delay 上，否则会先得出“无目标”的旧结论，等完仍按旧结论点击。等待期间不输入，结束后先检测“再次进行”，再查奖励候选；确认按钮消失则所有选卡/确认候选失败并安全等待。残留页重试重新经过此门控，选卡收尾到按钮确认不重复图标加载等待。不开关的默认确认及驱离无此等待。回归以分时合成画面覆盖红色图标晚 100/500/900ms 出现、始终无红色、等待时页面消失；1000ms 是有限宽限期，不是任意加载时长的保证。

`CipherEndlessRedReward`（“优先选择红色方块奖励”）只挂在 `CipherMode=Endless` 下，默认 No。No 显式禁用 `CipherEndlessRewardPage`、启用 `CipherEndlessRewardDefault`；Yes 相反。默认确认节点与 `RewardConfirmByClick` 的识别、坐标和时序完全一致，共用原二/三击及无输入收尾；关闭时不运行红色图标识别。开关只改两个无尽专属节点的 enabled，不覆盖共享入口或驱离确认节点；即使保存过 Yes，切换驱离后两种技能分支也不可达这些节点。测试覆盖 Yes/No 来回切换及与驱离覆盖的两种合并顺序、全部八种红色槽位组合、四种勾选状态不改变决策、鼠标遮挡或移除勾选/红色图标后仍只按确认按钮推进、标题/编号缺失、选卡后确认按钮消失安全等待；不改变三连击、计数、99 提醒或自然完成通知。

`tools/extract_cipher_reward_templates.py` 从保存的 `1294×730` 原图按客户区 `(10,8,1280,720)` 无损提取六个模板及客户区回归样本；保留自绘标题栏、不缩放，排除图标数量和持有数。其中旧标题、编号和勾选模板仅保留作参考，运行时不再加载，校验器也不再将其列为必需模板。原图与合成槽位调换/选中状态/亮暗场景覆盖识别分支；图可达性测试覆盖驱离隔离、三连击、重试及零额外计数。
